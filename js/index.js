...
nav.addEventListener('click', (event) => {alert('Still under construction!')})

// 2 Mouseover Navigation links red hover
const navA = document.querySelectorAll('nav a');

navA.forEach((link) => {link.addEventListener('mouseover', (event) => {
               event.target.style.color = 'red';
               setTimeout(function() { event.target.style.color = 'black'; },
                          500);
             }, false)});
...
card.addEventListener('dblclick', function(event) {
  ...
  setTimeout(function() { event.target.style.transform = 'scale(1.0)'; }, 500);
}, false)
...
advAwaits.addEventListener("click", (event)=>{
    ...
    h4.addEventListener("mouseenter", function(event) {
      event.target.style.color = "Red";
      setTimeout(function() { event.target.style.color = "Black"; }, 500);
    }, true);
    ...
    // 8 Mouse Down  Fun Bus color changes to red when mouse pointer is pushed
    // down click.
    const logo = document.querySelector('.logo-heading')
    logo.addEventListener(
        'mousedown', function(event) { event.target.style.color = "red"; });

    // 9 Mouse Leave  Color reverts back when mouse leaves 'Fun Bus'
    const logoRevert = document.querySelector('.logo-heading')
    logoRevert.addEventListener(
        'mouseleave', function(event) { event.target.style.color = "black"; });

    // 10 Changes the size of FUN IN THE SUN div class per mouse movement over
    // div class destination
    const logo2 = document.querySelector('.destination')
    logo2.addEventListener('mousemove', function(event) {
      event.target.style.transform = 'scale(1.2)';
      setTimeout(function() { event.target.style.transform = 'scale(1.0)'; },
                 500);
    }, false);
